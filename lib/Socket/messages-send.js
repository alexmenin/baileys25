...
const makeMessagesSocket = (config) => {
    ...
    const createParticipantNodes = async (jids, message, extraAttrs) => {
        // Criar fallback para mensagens interativas se necessário
        let messageToSend = message;
        if (shouldCreateFallback(message, [])) {
            const fallbackMessage = createInteractiveFallback(message);
            messageToSend = {
                ...message,
                conversation: fallbackMessage.text,
                ...message
            };
            
            logger.debug('Criando fallback para mensagem interativa individual', { 
                jids: jids.length,
                hasInteractive: true
            });
        }
        
        const patched = await patchMessageBeforeSending(messageToSend, jids)        
    ...
    const createInteractiveFallback = (message) => {
        let fallbackText = '';
        
        // Detectar tipo de mensagem interativa e criar fallback apropriado
        if (message.listMessage) {
            fallbackText = `📋 *${message.listMessage.title || 'Lista Interativa'}*\n\n`;
            
            if (message.listMessage.description) {
                fallbackText += `${message.listMessage.description}\n\n`;
            }
            
            if (message.listMessage.sections) {
                message.listMessage.sections.forEach((section, index) => {
                    if (section.title) {
                        fallbackText += `📂 *${section.title}*\n`;
                    }
                    
                    section.rows?.forEach((row, rowIndex) => {
                        fallbackText += `${rowIndex + 1}. ${row.title}`;
                        if (row.description) {
                    ...
                    if (index < message.listMessage.sections.length - 1) {
                        fallbackText += '\n';
                    }
                });
            }
            
            fallbackText += '\n💡 *Esta é uma lista interativa. Para vê-la corretamente, abra no WhatsApp Mobile.*';
            
        } else if (message.buttonsMessage) {
            fallbackText = `${message.buttonsMessage.contentText || message.buttonsMessage.headerText || 'Mensagem com Botões'}\n\n`;
            
            if (message.buttonsMessage.buttons) {
                fallbackText += '*Opções disponíveis:*\n';
                message.buttonsMessage.buttons.forEach((button, index) => {
                    fallbackText += `${index + 1}. ${button.buttonText?.displayText || 'Botão'}\n`;
                });
            }
            
            fallbackText += '\n💡 *Esta mensagem possui botões interativos. Para vê-los, abra no WhatsApp Mobile.*';
            
        } else if (message.interactiveMessage) {
            const interactive = message.interactiveMessage;
            
            if (interactive.header?.title) {
                fallbackText += `*${interactive.header.title}*\n\n`;
            }
            
            if (interactive.body?.text) {
                fallbackText += `${interactive.body.text}\n\n`;
            }
            
            if (interactive.footer?.text) {
                fallbackText += `_${interactive.footer.text}_\n\n`;
            }
            
            // Detectar tipo específico de interação
            if (interactive.nativeFlowMessage) {
                fallbackText += '*Ação Interativa Disponível*\n';
                
                if (interactive.nativeFlowMessage.buttons) {
                    interactive.nativeFlowMessage.buttons.forEach((button, index) => {
                        fallbackText += `${index + 1}. ${button.buttonParamsJson ? JSON.parse(button.buttonParamsJson).display_text || button.name : button.name}\n`;
                    });
                }
            } else if (interactive.listMessage) {
                fallbackText += '*Lista de Opções:*\n';
                interactive.listMessage.sections?.forEach(section => {
            ...
            fallbackText += '\n🔧 *Esta é uma mensagem interativa avançada. Para funcionalidade completa, use WhatsApp Mobile.*';
            ...
            if (template.hydratedTemplate) {
                ...
                if (hydrated.hydratedContentText) {
                    fallbackText += `${hydrated.hydratedContentText}\n\n`;
                }
                
                if (hydrated.hydratedButtons) {
                    fallbackText += '*Ações disponíveis:*\n';
                    hydrated.hydratedButtons.forEach((button, index) => {
                        if (button.quickReplyButton) {
                            fallbackText += `${index + 1}. ${button.quickReplyButton.displayText}\n`;
                        } else if (button.urlButton) {
                            fallbackText += `${index + 1}. ${button.urlButton.displayText} (${button.urlButton.url})\n`;
                        } else if (button.callButton) {
                            fallbackText += `${index + 1}. ${button.callButton.displayText} (${button.callButton.phoneNumber})\n`;
                        }
                    });
                }
            }
            
            fallbackText += '\n📱 *Esta é uma mensagem template. Alguns recursos podem não estar disponíveis neste dispositivo.*';
        }
    ...
    const shouldCreateFallback = (message, devices) => {
        // Detectar se mensagem é interativa
        const isInteractive = !!(
            message.listMessage ||
        ...
        // Para segurança, sempre criar fallback para mensagens interativas
        // pois não temos como detectar precisamente quais dispositivos suportam
        return true;
    };
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, additionalNodes }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                // Criar fallback para mensagens interativas se necessário
                let messageToSend = message;
                if (shouldCreateFallback(message, devices)) {
                    // Enviar versão interativa + versão fallback
                    const fallbackMessage = createInteractiveFallback(message);
                    
                    // Criar mensagem combinada com fallback
                    messageToSend = {
                        ...message,
                        // Adicionar fallback como texto alternativo
                        conversation: fallbackMessage.text,
                        // Manter a mensagem interativa original também
                        ...message
                    };
                    
                    logger.debug('Criando fallback para mensagem interativa', { 
                        hasListMessage: !!message.listMessage,
                        hasButtonsMessage: !!message.buttonsMessage,
                        hasInteractiveMessage: !!message.interactiveMessage,
                        hasTemplateMessage: !!message.templateMessage
                    });
                }
                
                const patched = await patchMessageBeforeSending(messageToSend, devices.map(d => WABinary_1.jidEncode(d.user, isLid ? 'lid' : 's.whatsapp.net', d.device)))                
                const bytes = Utils_1.encodeWAMessage(patched)
